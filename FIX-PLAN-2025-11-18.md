...
- [ ] Call `/api/sync/all` without admin → expect 403
- [ ] Call `/api/sync/all` as admin 3 times in 1 hour → expect 429 after 2nd call
...
- [ ] Test in development → verify sample logging works

### Success Criteria:
- ✅ Logs are readable (< 50 lines per PUSH)
- ✅ No PII exposed in production
- ✅ All data types captured
- ✅ Development mode still provides debugging info
...
**Priority**: CRITICAL
**Status**: ✅ DEPLOYED (commit 180e9a0)
**Deployed**: 2025-11-18
...
4. ✅ `FIX-PLAN-2025-11-18.md` - Comprehensive documentation

Deployment steps:
1. ✅ Commit changes (180e9a0)
2. ✅ Push to Railway
3. ⏳ Wait for Railway deployment
4. ⏳ Set ADMIN_USER_IDS environment variable in Railway
5. ⏳ Test authentication immediately
...
Tests:
- [ ] `/api/sync/manual` without userId → expect 401 Unauthorized
- [ ] `/api/sync/manual` with userId → expect 200 + sync results
- [ ] Call manual sync 6 times rapidly → expect 429 after 5th
- [ ] `/api/sync/all` without ADMIN_USER_IDS set → expect 403
- [ ] `/api/sync/all` with admin userId → expect 200
- [ ] `/api/logs` shows new requests (verify APILog fix)
- [ ] Send Garmin activity → check logs are concise (not 100MB)
